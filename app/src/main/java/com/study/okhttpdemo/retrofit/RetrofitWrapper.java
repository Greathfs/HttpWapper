...
public class RetrofitWrapper {
    private static WeakReference<RetrofitWrapper> sInstance;
    ...
    private RetrofitWrapper() {
        //缓存路径
        mCacheFile = new File(RetrofitHelper.getContext().getCacheDir().getAbsolutePath() + File.separator + "retrofit2_http_cache");
        //判断缓存路径是否存在
    ...
    public static RetrofitWrapper getInstance() {
        if (sInstance == null || sInstance.get() == null) {
            synchronized (RetrofitWrapper.class) {
                if (sInstance == null || sInstance.get() == null) {
                    sInstance = new WeakReference<RetrofitWrapper>(new RetrofitWrapper());
                }
            }
        }
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                try {
                    ...
                    if (LogWrapper.isDebug) {
                        LogWrapper.i("response data:" + json);
                    }
    ...
    private Interceptor createHttpInterceptor() {
        return new Interceptor() {
            ...
            public okhttp3.Response intercept(Chain chain) throws IOException {
                ...
                boolean isNetwork = isNetworkConnected(RetrofitHelper.getContext());
